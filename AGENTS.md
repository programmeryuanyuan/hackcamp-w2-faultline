...
- web/public/       : snapshot.json（worker 写、web 读）
- contracts/        : Solidity 合约（Remix 部署，不进构建链）
...
需先告知（L1）: 修改 package.json dependencies / scripts、重命名 public export
必须先问（L0 — 绝对禁止）:
  - 主网 / 涉及真实资产的交易
  - 读取或写入 .env / 私钥 / 助记词文件
  - 提交 .env 到 git（.gitignore 已配置，绝不 --force-add）
  - 强制 push / 删除 git 分支
  - 跳过 lint / test / pre-commit hook
